...
while True:
    ...
    try:
        if message.startswith("remember "):
            ...
            for step in plan["steps"]:
                ...
                if step_type == "MEMORY":
                    ...
                    result = execute_tool(step["action"], step.get("target"))
                    if result is False:
                    ...
                    query = step["content"] or message
                    memories = get_relevant_memories(query)
                    prompt = build_memory_prompt(query, memories) if memories else query
                    print("JARVIS [Local]: ", end="")
                    ask_llm(prompt)
        ...
        # Queue only after response execution/output completes. The daemon
        # writer keeps SQLite work off the response path.
